...
public class GraphGenerator extends TWayGraphProblem
{
  ...
  /**
   * Computes the ID of a vertex, based on the valuation of the variables it
   * represents
   * 
   * @param tuple
   *          The names of the variables in this tuple
   * @param tuple_nb
   *          The sequential number given to this combination of variables
   * @param values
   *          A list of values; the i-th position of the list contains the value
   ...
   * 
   * @param elements
   *          A list of strings, must be of length t &times; 2. Each pair of
   *          arguments represents the name of a variable, followed by the value
   *          of that variable. So, to represent the pair a=1, b=2, the arguments
   *          would be ("a", "1", "b", "2).
   * @return The numerical ID of the corresponding vertex
   */
          ...
          while (vi.hasNext() && m_running)
          {
            ...
            while (vi2.hasNext() && m_running)
            {
              ...
              if (new_vertex_id < cur_vertex_id)
              {
                ...
                edgeStart();
                vertexCallback(cur_vertex_id, tuple, tuple_pos, valuation, new_vertex_id,
                    previous_tuple, i, conflict_valuation);
                edgeEnd();
              }
            }
          }
  ...
  /**
   * Checks if a valuation for variables has the same assignments than that of a
   * given tuple
   * 
   * @param tuple
   *          The variables in the tuple
   * @param valuation
   ...
   * @return {@code true} if all variables in common_vars have the same value as
   *         in tuple, {@code false} otherwise
   */
  protected boolean sameValues(List<String> tuple, List<String> valuation, List<String> common_vars,
      List<String> values)
  {
