...
public class FileHelper
{
   ...
   * @param f
   *          The file to read
   * @return The string with the file's contents, or the empty string if an error
   *         occurred.
   */
  ...
  /**
   * Replace the extension of a filename with another. For example, one can
   * replace /my/path/foo.bar with /my/path/foo.baz.
   * 
   * @param filename
  ...
  /**
   * Trims the extension of a filename. For example, with /my/path/foo.bar, would
   * return /my/path/foo
   * 
   * @param filename
   *          The filename
   ...
   * 
   * @param c
   *          The class used as reference. The path is expressed relative to the
   *          location of this class in the project.
   * @param path
   *          The path
   * @return The string, or {@code null} if the path does not correspond to a
   *         resource
   */
   ...
   * @param path
   *          The path
   * @return An input stream, or {@code null} if the path does not correspond to a
   *         resource
   */
   ...
   * @return The file's contents, and empty string if the file does not exist
   * @throws IOException
   ...
   *          The pattern to match
   * @param exclude_glob
   *          The pattern to exclude files
   * @param exclude_dirs
   *          Set to {@code true} to ignore entries that are directories
   * @return Just the name of each member item, not the full paths.
   * @throws URISyntaxException
   * @throws IOException
   */
      ...
      if (dirURL.getProtocol().equals("jar"))
      {
        ...
        while (entries.hasMoreElements())
        {
          ...
          { // filter according to the path
            ...
            /*
             * int checkSubdir = entry.indexOf("/"); if (checkSubdir >= 0) { // if it is a
             * subdirectory, we just return the directory name entry = entry.substring(0,
             * checkSubdir); }
             */
   ...
   * 
   * @param o
   *          The object used as reference. The path is expressed relative to the
   *          location of the object's declaring class in the project.
   * @param filename
   *          The path
   * @return The string, or {@code null} if the path does not correspond to a
   *         resource or if {@code o} is null
   */
